...
def generar_comprobante(tipo_de_comprobante: int, tipo_documento: str, numero_documento: str, pedido_id: int):
    ...
    pedido:Pedido|None = Pedido.objects.filter(id=pedido_id).first()
    validar_existencia = Comprobante.objects.filter(pedido = pedido.id).first()
    if(validar_existencia is not None): raise Exception('Pedido ya tiene un comprobante')
    '''try:
        if (pedido.comprobante is not None): raise Exception('Pedido ya tiene un comprobante')
